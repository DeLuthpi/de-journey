...
const EditModal = ({ showEditModal, setShowEditModal, selectedBanner }) => {
	...
	const handleSubmit = async (e) => {
		...
		const payload = {
			...
			imageUrl: imageUrlUploaded || selectedBanner?.imageUrl,
		};
		...
		const res = await postData(`update-banner/${selectedBanner.id}`, payload);
		if (res.status === 200) {
				...
				{(onClose) => (
					<>
						<ModalHeader className="flex flex-col gap-1">
							<div className="mx-2 my-4 align-center">Update Banner</div>
						</ModalHeader>
