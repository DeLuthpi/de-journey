...
const EditModal = ({ showEditModal, setShowEditModal, selectedDestination }) => {
	...
	const handleSubmit = async (e) => {
		...
		const payload = {
			...
			imageUrls: imageUrlUploaded || selectedDestination?.imageUrls,
		};
				...
				{(onClose) => (
					<>
						...
						<form className="mb-5 space-y-2 md:space-y-3" onSubmit={handleSubmit}>
							<ModalBody>
								<img width={200} height={200} alt="image" className="w-full rounded-lg object-cover h-[165px]" src={imageUrlUploaded === null ? (validateImg(selectedDestination?.imageUrls) ? selectedDestination?.imageUrls : noImage) : imageUrlUploaded} />
								<Input type="text" defaultValue={selectedDestination?.title} id="title" size="sm" label="Title" name="title" isRequired className="max-w-full" onChange={(e) => setTitle(e.target.value)} autoComplete="off" variant="bordered" color="primary" autoFocus />
								<input type="file" accept="image/*" onChange={handleUpload} id="dataimage" name="dataimage" className="block w-full px-1 py-2 mb-5 text-sm border-2 border-gray-200 rounded-lg cursor-pointer text-primary hover:border-gray-400 bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" />
