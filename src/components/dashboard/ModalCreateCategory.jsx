...
const CreateModal = ({ showCreateModal, setShowCreateModal }) => {
	...
	const handleSubmit = async (e) => {
		...
		const res = await postData("create-category", payload);
		if (res.status === 200) {
				...
				{(onClose) => (
					<>
						<ModalHeader className="flex flex-col gap-1">
							<div className="mx-2 my-4 align-center">Create Category</div>
						</ModalHeader>
						<form className="mb-5 space-y-2 md:space-y-3" onSubmit={handleSubmit}>
							<ModalBody>
								<img width={200} height={200} alt="image" className="w-full rounded-lg object-cover h-[165px]" src={imageUrlUploaded ? imageUrlUploaded : addImage} />
								<Input type="text" id="name" value={name} size="sm" label="Name" name="name" isRequired className="max-w-full" onChange={(e) => setName(e.target.value)} autoComplete="off" variant="bordered" color="primary" autoFocus />
