...
const EditModal = ({ showEditModal, setShowEditModal, selectedPromo }) => {
	...
	const handleSubmit = async (e) => {
		...
		const payload = {
			...
			imageUrl: imageUrlUploaded || selectedPromo?.imageUrl,
		...
		const res = await postData(`update-promo/${selectedPromo?.id}`, payload);
		if (res?.status === 200) {
				...
				{(onClose) => (
					<>
						<ModalHeader className="flex flex-col gap-1">
							<div className="mx-2 my-4 align-center">Edit Promo</div>
						</ModalHeader>
								...
								<div className="flex flex-wrap gap-2 md:flex-nowrap">
									<div className="flex flex-wrap w-full mx-auto md:w-2/5">
										<div className="overflow-hidden rounded-md w-full h-[240px]">
											<img src={imageUrlUploaded === null ? (validateImg(selectedPromo?.imageUrl) ? selectedPromo?.imageUrl : noImage) : imageUrlUploaded} className="object-cover object-center w-full h-full scale-100 rounded-md" alt="img-show" />
										</div>
									...
									<div className="flex flex-wrap w-full space-y-1.5 md:space-y-3 md:w-3/5">
										<div className="flex flex-wrap w-full gap-1.5 lg:gap-2 md:flex-nowrap">
											<Input type="text" id="title" size="sm" label="Title" name="title" defaultValue={selectedPromo?.title} className="max-w-[49%] max-[375px]:max-w-full md:max-w-full" autoComplete="off" variant="bordered" color="primary" autoFocus />
											<Input type="text" id="promo_code" size="sm" label="Promo Code" name="promo_code" defaultValue={selectedPromo?.promo_code} className="max-w-[49%] max-[375px]:max-w-full md:max-w-full" autoComplete="off" variant="bordered" color="primary" />
										</div>
										...
										<div className="flex flex-wrap w-full">
											<Textarea
												...
												defaultValue={selectedPromo?.terms_condition}
												disableAnimation
												disableAutosize
												classNames={{
													base: "max-w-full",
										...
										<div className="flex flex-wrap w-full gap-1.5 lg:gap-2 md:flex-nowrap">
											<Input type="number" id="price" name="discount_price" defaultValue={discountPrice === "" ? selectedPromo?.promo_discount_price : discountPrice} label="Discount Price" onChange={(e) => setDiscountPrice(e.target.value.replace(/[^0-9]/g, ""))} size="sm" className="max-w-[49%] max-[375px]:max-w-full md:max-w-full" variant="bordered" color="primary" />
											<Input type="number" id="discount" name="claim_price" defaultValue={claimPrice === "" ? selectedPromo?.minimum_claim_price : claimPrice} label="Claim Price" onChange={(e) => setClaimPrice(e.target.value.replace(/[^0-9]/g, ""))} size="sm" className="max-w-[49%] max-[375px]:max-w-full md:max-w-full" variant="bordered" color="primary" />
										</div>
									</div>
								</div>
							</ModalBody>
