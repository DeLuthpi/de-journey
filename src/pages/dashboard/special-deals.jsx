...
import CreateModal from "@/components/Dashboard/ModalCreatePromo";
import EditModal from "@/components/Dashboard/ModalEditPromo";
import DeleteModal from "@/components/Dashboard/ModalDeletePromo";
...
const SpecialDealsPage = () => {
	...
	useEffect(() => {
		getData("promos", (res) => {
			const dataFiltered = res?.data.data.filter((promo) => promo?.name.toLowerCase().includes(search.toLowerCase()));
				...
				<div className="w-full p-6 mx-auto">
					<div className="flex flex-wrap -mx-3">
						<div className="w-full max-w-full px-3 my-auto shrink-0 md:flex-0 md:w-2/4 lg:w-2/3">
							<h5 className="mb-0 text-2xl font-semibold dark:text-white">Promo List</h5>
						</div>
							...
							<div key={index} className="w-full max-w-full px-3 mb-4 transition-all duration-700 ease-in-out shrink-0 sm:flex-0 md:w-6/12 lg:w-4/12 hover:scale-105">
								<div className="relative flex flex-col min-w-0 border-0 break-word rounded-2xl bg-clip-border">
									<Card isFooterBlurred isPressable className="w-full h-[250px] col-span-12 sm:col-span-7">
										...
										<Image removeWrapper alt={list?.title} className="z-0 object-cover w-full h-full" src={validateImg(list?.imageUrl) ? list?.imageUrl : noImage} fallbackSrc={noImage} />
					...
					<EditModal showEditModal={showEditModal} setShowEditModal={setShowEditModal} selectedPromo={selectedPromo} />
					<DeleteModal showDeleteModal={showDeleteModal} setShowDeleteModal={setShowDeleteModal} selectedPromo={selectedPromo} />
				</div>
			</main>
		</div>
	);
